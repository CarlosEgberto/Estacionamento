...
function imprimirComprovante() {
    // Esconder o modal antes de imprimir
    fecharComprovante();
    ...
    janelaImpressao.document.write(`
        <html>
            <head>
                <title>Comprovante Estacionamento</title>
                <style>
                    @page { margin: 20px; }
            ...
            <body>
                ...
                <svg class="barcode">${document.getElementById('barcode').innerHTML}</svg>
                <div class="info">
                    <p>ID: ${document.getElementById('compId').textContent}</p>
                    <p>Placa: ${document.getElementById('compPlaca').textContent}</p>
                    <p>Modelo: ${document.getElementById('compModelo').textContent}</p>
                    <p>Entrada: ${document.getElementById('compEntrada').textContent}</p>
                </div>
            </body>
        </html>
    `);
...
function registrarSaida() {
    ...
    if (confirmar) {
        vehicles = vehicles.map(v => 
            v.id === selectedVehicle.id ? {...v, ativo: false} : v
        );
        localStorage.setItem('vehicles', JSON.stringify(vehicles));
...
window.onload = () => {
    renderVeiculos();
    // Restaurar contador se necessário
